...
class Server(db.Model):
    ...
    def check_mandatory_volumes(self):
            ...
            try:
                ...
                print("self.mandatory_volumes in form = {}".format(self.mandatory_volumes))
                print("self.mandatory_volumes after split= {}".format(volumes))
                if volumes != [] and volumes != ['']:
                    print("mandatory volumes exists")
                    return volumes
                else:
                    return False
            except:
    ...
    def check_mounted_all_volumes(self):
        volumes_mandatory  = self.check_mandatory_volumes() 
        i = 0
        if volumes_mandatory:
            for vol in volumes_mandatory:
